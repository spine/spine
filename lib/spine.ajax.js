(function() {
  ...
  Ajax = Spine.Ajax = {
    ...
    success: function(data, status, xhr) {
      ...
      if (data.id && record.id(!data.id)) {
        records = record.__super__.records;
    ...
    send: function(record, method, params) {
      ...
      if (method === "create" && record.model) {
        params.url = this.getUrl(record.__super__);
      } else {
  ...
  Model.Ajax = {
    extended: function() {
      this.sync(function() {
        return Ajax.ajaxSync.apply(Ajax, arguments);
      });
      this.fetch(__bind(function(params) {
        return Ajax.ajaxSync(this, "read", params);
      }, this));
